...
pub fn run_command(program: &str, args: &[&str]) -> String {
    ...
    Command::new(program).args(args).stdout(Stdio::piped()).output().unwrap().stdout.into_iter().map(|b| b as char).collect()
...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    Command::new("cat").stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().and_then(|mut child| {
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    Command::new("sh").args(["-c", command]).status().unwrap().code().unwrap_or(1)
}
...
pub fn run_command_with_result(program: &str, args: &[&str]) -> io::Result<String> {
    ...
    Command::new(program).args(args).stdout(Stdio::piped()).output().and_then(|output| {
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    Command::new("grep").args([pattern]).stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().and_then(|mut child| {
