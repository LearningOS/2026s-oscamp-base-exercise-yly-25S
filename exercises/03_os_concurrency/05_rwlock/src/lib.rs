    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            ...
            if state & READER_MASK != 0 || state & WRITER_HOLDING != 0 {
                ...
                if self.state.compare_exchange(WRITER_WAITING, WRITER_HOLDING, Ordering::Release, Ordering::Relaxed).is_ok() {
                    return RwLockWriteGuard { lock: self };
                } else if self.state.compare_exchange(0, WRITER_HOLDING, Ordering::Release, Ordering::Relaxed).is_ok() {
                    return RwLockWriteGuard { lock: self };                   
                }
            }
        }
