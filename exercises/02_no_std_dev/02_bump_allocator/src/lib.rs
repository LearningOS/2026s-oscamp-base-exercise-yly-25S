...
unsafe impl GlobalAlloc for BumpAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        let this_next = self.next.load(Ordering::SeqCst);
        ...
        if end > self.heap_end {
            ...
            loop {
                if self.next.compare_exchange(this_next, end, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
                    break this_next_aligned as *mut u8;
                }
            }
        }
    }
